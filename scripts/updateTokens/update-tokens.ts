...
function githubApi<T extends object>(pathTail: string): Promise<T> {
  return httpsGet({
    ...
    path: `/repos/sophon-lists/tokens${pathTail}`,
    headers: {
      "user-agent": "node",
...
async function getIPFSaddresses() {
  ...
  if (useHardcodedAddresses) {
    ...
    // First we fetch the latest commit from sophon-lists/tokens
    console.log("Fetching sophon-lists/tokens commits...");
...
async function run() {
  ...
  networks.forEach(async network => {
    if (!!ipfsAddresses[network.networkNameIPFS]) {
      ...
      // Format the json to match our format in /packages/leona-react/src/assets/tokens/<network>.json
      const tokens = await processTokenJson(tokensJson);
      // Write to the file
      console.log(
        `Writing Tokens JSON to /packages/leona-react/src/assets/tokens/${
          network.networkName
        }.json...`
      );
      const filePath = path.resolve(
        __dirname,
        `../../packages/leona-react/src/assets/tokens/${
          network.networkName
        }.json`
      );
      fs.writeFile(
