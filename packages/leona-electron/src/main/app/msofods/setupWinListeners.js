...
const { TRUSTED_HOSTS } = SECURITY_OPTIONS.leonaNetwork;
const trustedHostsAll = Object.values(TRUSTED_HOSTS).flat();
const pino = Pino();

function setupWinListeners (leonaApp) {
  ...
  win.on('moved', () => {
     ...
     * we run it just in 'close' instead, then if the Leona app
     * crashes after they've moved the Leona window then it won't run
  ...
  win.on('closed', () => {
    ...
    leonaApp.emit('after-closed-window');
  });

  win.on('minimize', () => {
    leonaApp.emit('minimize-window');
  });
}
