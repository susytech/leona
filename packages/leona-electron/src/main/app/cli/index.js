...
cli
  ...
  .option(
    '--no-run-susy',
    `${productName} will not attempt to run the locally installed susy.`
  )
  .option(
    '--ws-port <port>',
  ...
  .parse(
    process.argv
      // We want to ignore some flags and not pass them down to Susy:
      // --inspect: `electron-webpack dev` runs Electron with the `--inspect` flag for HMR
      // -psn_*: https://octonion.institute/susytech/leona/issues/188
      // --ws-interface: we don't pass down this flag, because leona only allows 127.0.0.1 as WS interface
      .filter(
        arg =>
