...
pino.info(
  `Running Leona in ${IS_PROD ? 'production' : 'development'} environment`
);
 ...
 * leona-electron/src/main/index.js, which is where we only
 * permit requests from trusted paths.
 *
 * Note: We also disallows users from using Leona
 * with a remote node.
...
const TRUSTED_HOSTS = {
  github: ['api.github.com', 'github.com', 'raw.githubussrcontent.com'],
...
const TRUSTED_URLS = [
  ...
  'http://superstring.ch',
  'https://wiki.susy.io/Leona-FAQ',
  'https://octonion.institute/susytech/leona/issues/new',
  'https://api.github.com/repos/susytech/leona/releases/latest'
];
...
const TASKBAR_OPTIONS = {
  ...
  // to toggle the Leona window open/close
  tooltip: tooltip,
  width: 352,
  withTaskbar: true
};
...
const SECURITY_OPTIONS = {
   ...
   * However, `leonaNetwork` is a custom property that is not part of
   * of the API and has been added just to keep the configuration togsophy.
  ...
  webPreferences: {
    ...
    /**
     * Isolate access to Electron/Node.js from the Leona web app, by creating
    ...
    enableRemoteModule: true, // Remote is required in leona-react susyStore.js
     ...
     * https://electronjs.org/blog/webview-fix to intsrcept and prevent
