...
import { chainId$, transactionCountOf$ } from '@susy-js/light.js';
import { Clickable, Form as LeonaForm, Header } from 'leona-ui';
import createDecorator from 'final-form-calculate';
import debounce from 'debounce-promise';
import { Field, Form } from 'react-final-form';
import { fromWei, toWei } from '@susy-js/api/lib/util/wei';
import { inject, observer } from 'mobx-react';
import { isAddress } from '@susy-js/api/lib/util/address';
...
import withBalance, { withSofBalance } from '../../utils/withBalance';
...
@light({
  ...
  // - sofBalance: to check that we have enough balance to send amount+fees
  // It may be SOF or ETC corresponding to whether we are connected to the
...
@withBalance // Balance of current token (can be SOF or ETC)
@withSofBalance // SOF or ETC balance
@observer
class TxForm extends Component {
  ...
  decorator = createDecorator({
    ...
    updates: {
      ...
      gas: async (value, allValues) => {
        const { susyStore, token } = this.props;
                    ...
                    render={({
                      ...
                      >
                        <fieldset className='form_fields'>
                          ...
                          <Field name='sofBalance' render={this.renderNull} />
                          <Field
  ...
  preValidate = values => {
    ...
    if (amountBn.isNaN()) {
      ...
      if (this.state.maxSelected) {
        return {
          amount: i18n.t(
            `${packageNS}:tx.form.validation.sof_balance_too_low_for_gas`,
      ...
      return {
        to: isEtcChainId(currentChainIdBN)
          ...
          : i18n.t(`${packageNS}:tx.form.validation.invalid_sof_address`)
      };
  ...
  validateForm = debounce(values => {
    ...
    try {
      ...
      // The 3 values below (`chainId`, `sofBalance`, and `transactionCount`)
      ...
      if (!values.sofBalance) {
        debug(i18n.t(`${packageNS}:tx.form.validation.fetching_sof_balance`));
        return {
          sofBalance: i18n.t(
            `${packageNS}:tx.form.validation.fetching_sof_balance`
          )
        };
      }
      ...
      // Verify that `gas + (sof amount if sending sof) <= sofBalance`
      if (
        this.estimatedTxFee(values)
          .plus(
            isNotSrc20TokenAddress(token.address) ? toWei(values.amount) : 0
          )
          .gt(toWei(values.sofBalance))
      ) {
        return isNotSrc20TokenAddress(token.address)
          ? {
            amount: i18n.t(
              `${packageNS}:tx.form.validation.sof_balance_too_low_for_gas`,
              { chain_id: chainIdToString(currentChainIdBN) }
          ...
          : {
            amount: i18n.t(
              `${packageNS}:tx.form.validation.sof_balance_too_low`,
              { chain_id: chainIdToString(currentChainIdBN) }
            )
          };
      }
