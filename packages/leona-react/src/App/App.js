...
import RequireSusyVersion from '../RequireSusyVersion';
...
@inject('onboardingStore', 'susyStore')
@observer
class App extends Component {
  ...
  componentDidMount () {
    ...
    window
      .fetch('https://api.github.com/repos/susytech/leona/releases/latest')
      .then(j => j.json())
      ...
      .catch(e => {
        console.error('Error while checking for a new version of Leona:', e);
      });
  }
  ...
  render () {
    ...
    // We set susyStore.api right after we set the API for light.js, so we
    // verify here that susyStore.api is defined, and if not we don't render
