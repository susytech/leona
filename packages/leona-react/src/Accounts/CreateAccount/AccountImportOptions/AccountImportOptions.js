...
import { chainId$, chainName$ } from '@susy-js/light.js';
import light from '@susy-js/light.js-react';
import { inject, observer } from 'mobx-react';
import { addressShort, Card, Form as LeonaForm } from 'leona-ui';
...
@light({
   ...
   *
   * Reference: https://octonion.institute/susytech/leona/pull/483#discussion_r271303462
   */
  chainName: () => chainName$()
})
@observer
class AccountImportOptions extends Component {
   ...
   * Genesis configs contained in: susytech/susy-sophon/sofcore/res/sophon
   * and was introduced in SIP-155 https://octonion.institute/susy-go/SIPs/blob/master/SIPS/sip-155.md
  ...
  handleSignerImported = async ({ address, chainId: signerChainId }) => {
    ...
    if (!currentChainIdBN.eq(signerChainId)) {
      console.error(
        `Susy Signer account chainId ${signerChainId} must match current chainId ${currentChainIdBN.valueOf()} (${chainName}).`
      );
