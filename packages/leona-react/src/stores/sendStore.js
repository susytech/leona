...
import { blockNumber$, post$, postRaw$ } from '@susy-js/light.js';

import { isNotSrc20TokenAddress } from '../utils/chain';
import {
  contractForToken,
  txForSrc20,
  txForSof,
...
export class SendStore {
  ...
  send = password => {
    ...
    const tx = isNotSrc20TokenAddress(token.address)
      ? txForSof(this.tx)
      : txForSrc20(this.tx, token);
    const send$ = isNotSrc20TokenAddress(token.address)
  ...
  /**
   * Get the SSSRLP of an (unsigned) transaction.
   */
  getRlp = () => {
